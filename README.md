...
- **Admin content workspace:** `/yonetim/icerikler` — LearningContent list/detail/create (platform drafts). Draft revision block editor at `/yonetim/icerikler/{id}/revision` (typed fields only; admin preview at `.../revision/onizleme`). Catalog subject show maps canonical Subject by UUID only (unmap blocked when placements/LC bound). Access policy fail-closed by default; Free requires explicit confirm + audit.
