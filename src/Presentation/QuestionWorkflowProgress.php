...
/**
 * Role-aware next step for the question editor. Does not grant permissions.
 */
final class QuestionWorkflowProgress
{
    /**
     * @param array{
     *     status: string,
    ...
    public function summarize(array $state): array
    {
        ...
        if ($archived) {
            $next = ['kind' => 'none', 'label' => 'Arşivde', 'hint' => 'Arşivlenmiş soru yeniden açılmaz.'];
            ...
            $next = ['kind' => 'edit', 'label' => 'Taslağı tamamla', 'hint' => 'Soru kökünü ve seçenekleri kaydedin.'];
        } elseif ($inReview && $state['is_revision_author']) {
            $next = ['kind' => 'wait', 'label' => 'İnceleme bekleniyor', 'hint' => 'Kendi hazırladığınız soruyu yayınlayamazsınız.'];
        } elseif ($inReview && $state['can_publish']) {
            $next = ['kind' => 'publish', 'label' => 'Yayınla', 'hint' => 'Yayınlanan sürüm değiştirilemez.'];
