...
- **Kurum actor sertleştirmesi:** Kurum domainindeki bütün yetkili işlemlerde actor **active + e-posta doğrulanmış** olmalıdır. `ROLE_SUPER_ADMIN` override, suspended / archived / pending_verification hesabı **bypass etmez**. Kritik yazma servisleri (`InstitutionCreator`, `InstitutionStatusManager`, `InstitutionMembershipManager`) transaction içinde actor/subject User satırını UUID ile **fresh reload** eder (`PESSIMISTIC_READ`); metoda verilen stale entity’nin role/status değerlerine güvenilmez. Kilit sırası: **1) Institution (WRITE)** → **2) User’lar UUID artan sırada (READ)** → **3) Membership (WRITE)**. Kurum oluşturmada henüz institution satırı yoksa yalnızca User kilitleri (UUID sırası) alınır; aynı actor=owner satırı bir kez kilitlenir. `UserGlobalRoleManager` / `UserStatusManager` bu aşamada pessimistic user lock kullanmaz; institution akışları kendi sırasını uygular. Deadlock / lock timeout güvenli `conflict` reason’a çevrilir. `InstitutionVoter` token User’ı request-scope `RequestScopedUserLookup` ile DB’den okur (write lock yok); active+verified kontrolü SUPER_ADMIN override’dan **önce** çalışır. Membership role ile global role ayrımı korunur.
